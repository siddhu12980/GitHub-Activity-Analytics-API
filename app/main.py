...
async def lifespan(app: MyFastAPI):
    try:
         ...
         app.GITHUB_CLIENT_ID = os.getenv('GITHUB_CLIENT_ID')

         app.GITHUB_CLIENT_SECRET = os.getenv('GITHUB_CLIENT_SECRET')

         app.GITHUB_REDIRECT_URI = os.getenv('GITHUB_REDIRECT_URI')
...
async def auth_with_github(code:str ):
    ...
    data ={
        ...
        "redirect_uri":app.GITHUB_REDIRECT_URI,
    }
...
def get_user_events_long(token: str = Depends(get_github_token), user_info: Dict[str, any] = Depends(get_user_details)):
    ...
    res =  helper.parse.parse_github_events(events_data)
